...
if (fetchSize != null && fetchSize > 0) {
    ...
    ps.setFetchSize(100); // Guard blocks Integer.MIN_VALUE
}
```
---
...
Under the explicitly defined scope (no `useCursorFetch`), Hypothesis H is REFUTED and Hypothesis A is CONFIRMED.** When stripped of cursor-based safety nets, the Moqui iterator does *not* stream row-by-row. Instead, the MySQL Connector/J ignores the coerced positive fetch size (100) and buffers the entire result set into the JVM heap upfront, resulting in a catastrophic `OutOfMemoryError`.
